...
export const load: PageServerLoad = ({ params }) => {
	...
	if (role.official_status === 'official_occupation_match' && role.official_occupation) {
	...
	const related = syntheticRolesV9
		.filter(
			candidate =>
				candidate.official_status === 'non_official_role_query' &&
				candidate.slug !== role.slug &&
				roleCategoryMap[candidate.slug]?.category === category
		)
		.sort((a, b) => {
			const aRank = a.estimate?.estimated_comparison_percentile ?? -1;
			const bRank = b.estimate?.estimated_comparison_percentile ?? -1;
