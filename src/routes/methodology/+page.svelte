...
<main class={pageLayout({ width: 'prose' })}>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-3 text-sm text-muted-foreground">
			These three layers produce sub-scores, a net risk band, and a visible confidence level — not a
			single magic number.
		</p>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground italic">
			The market modifier only dampens risk (it is always &le; 1.0). Weak markets don't amplify risk
			beyond the technical signal — they simply provide less buffer. Maximum reduction: 35%.
		</p>
	</section>
	...
	<section class="mb-8 mt-8">
		...
		<div class={cn(card({ padding: 'md' }), 'mt-4')}>
			...
			<p class="mt-1 text-sm text-muted-foreground">
				...
				occupations via O*NET. Measures how much a job's required abilities overlap with current AI
				capabilities. Available for ~774 US occupations by 6-digit SOC code.
			</p>
			<p class="mt-2 rounded bg-muted px-3 py-2 font-mono text-sm text-foreground/80">
	...
	<section class="mb-8 mt-8">
		...
		<div class={cn(card({ padding: 'md' }), 'mt-4')}>
			...
			<ol class="mt-2 list-inside list-decimal space-y-1 text-sm text-muted-foreground">
				...
				<li>
					...
					<code class="rounded bg-muted px-1 text-xs">4.C.3.b.2</code> (inverted), Structured vs.
					Unstructured Work
			...
			<p class="mt-2 text-sm text-muted-foreground">
				Scale: 0 (lowest percentile — most automatable) to 1 (highest percentile — strongest human
				bottleneck)
			</p>
		</div>
		...
		>
			<p class="text-sm text-foreground/80">
				...
				because multiplying by <code>(1 - theta)</code> would double-count complementarity. C-AIOE is
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			...
			lagging and confounded. The market layer can reduce net risk by up to 35% — it never amplifies
			risk beyond the technical signal.
		</p>
		...
		<div class="mt-4 space-y-4">
			<div class={card({ padding: 'md' })}>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					10-year employment CAGR (2015-2025) and 8-year wage CAGR (2015-2023) from MOM data, per
					major occupation group. Percentile-ranked across the 8 major groups.
				</p>
			</div>
			...
			<div class={card({ padding: 'md' })}>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					Two wage-structure signals: (1) log wage spread (winsorized at 1st/99th percentile) as a
					scarcity proxy, and (2) within-group wage position (above group median = relative
			...
			<div class={card({ padding: 'md' })}>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					Group-level trends get 60% weight (direct measurement). Occupation-level wage structure
					adds within-group differentiation at 40% weight (noisier signal). The 0.35 cap means the
					market layer can reduce net risk by up to 35%.
				</p>
			</div>
			...
			<div class={card({ padding: 'md' })}>
				...
				<ul class="mt-2 list-inside list-disc space-y-1 text-sm text-muted-foreground">
					<li>
						<strong>Shortage Occupation List (SOL) 2026</strong>: EP/COMPASS shortage list (released
						Nov 2025, effective Jan 2026). 36 occupations across semiconductors, healthcare, ICT,
						green economy, maritime, agritech, financial services. Matched to 62 SSOC codes.
						<strong>+15% market resilience bonus.</strong>
					</li>
					<li>
						<strong>Jobs in Demand 2025</strong>: Broader resident demand list (released Dec 30,
						2025). Covers PMET and non-PMET roles: software developers, nurses, waiters, drivers,
						security officers, auditors. Matched to 29 SSOC codes.
			...
			<div class={card({ padding: 'md' })}>
				...
				<p class="mt-2 text-sm text-muted-foreground italic">
					...
					labour market card on occupation pages. It is not a direct scoring input — we show it to
					ground the analysis in current labour-market reality without overfitting a sparse
					cluster-level series into the per-occupation formula.
				</p>
				<p class="mt-2 text-sm text-muted-foreground">
					<strong>Data displayed per cluster:</strong> vacancy rate with year-over-year trend and multi-year
					sparkline (2021–2025), recruitment vs resignation rates, retrenchment incidence per 1,000 employees,
					and re-entry rates (proportion of retrenched workers finding employment within 6 and 12 months).
				</p>
				<p class="mt-2 text-sm text-muted-foreground italic">
			...
			<div class={card({ padding: 'md' })}>
				...
				<p class="mt-2 text-sm text-muted-foreground">
					...
					Key finding: some occupations (data entry, customer service) show higher observed usage than
					AIOE predicts, while others (teachers, software developers) show lower observed usage.
				</p>
				<p class="mt-2 text-sm text-muted-foreground italic">
					Used as calibration, not replacement. Anthropic measures Claude usage specifically, not
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-3 text-sm text-muted-foreground italic">
			...
			displacement and official demand signals (SOL/JiD) are classified as "mixed" rather than "at
			risk".
		</p>
	</section>
	...
	<section class="mb-8 mt-8">
			...
			<table class="w-full text-left text-sm">
				...
				<tbody class="text-muted-foreground">
					<tr class="border-b border-border/50">
						...
						<td class="py-2 pr-3"
							>Direct = 1.0, sub-major fallback = 0.6, major fallback = 0.3, then reduced further by
							crosswalk dispersion where mapped SOC scores disagree.</td
						>
					...
					<tr class="border-b border-border/50">
						...
						<td class="py-2 pr-3"
							>Baseline = occupation wage structure + group employment/wage trends. Exact official
							demand evidence adds more occupation-specific Singapore signal than prefix-inferred or
							absent demand evidence.</td
						>
					...
					<tr>
						...
						<td class="py-2 pr-3"
							>Baseline reflects a mix of 2021 academic exposure data and recent Singapore labour
							data. Anthropic observed-usage calibration raises freshness where available.</td
						>
		...
		<p class="mt-2 text-sm text-muted-foreground italic">
			...
			prefix-inferred demand evidence, and whether Anthropic observed-usage calibration is available
			for the matched occupation.
		</p>
	</section>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			Building on the IMF framework, occupations are classified into four impact types based on the
			2&times;2 matrix of displacement risk and augmentation potential:
		</p>
		<div class="mt-3 space-y-2">
			...
			<div class="flex items-start gap-3">
				...
				<div>
					...
					<p class="text-sm text-muted-foreground">
						High displacement, high augmentation — conflicting signals, outcome depends on adoption
						path
					</p>
				</div>
			</div>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			...
			occupation-level wage structure). Net risk is published as risk bands with visible confidence.
			Augmentation potential, impact type classification, and rule-based outlook/scenario modelling
			are included. 80 estimated modern roles (AI engineer, product manager, prompt engineer, etc.)
			are scored as weighted blends of official occupations, with dispersion analysis for
			high-variance compositions.
		</p>
		<p class="mt-2 text-sm text-muted-foreground">
			...
			sensitivity — roles with high institutional knowledge (e.g., software engineering) vary more by
			seniority than roles with low context-dependence (e.g., truck driver). Grounded in: Stanford "Canaries
			in the Coal Mine" (2025) showing entry-level displacement pressure, and Anthropic Economic Index
			(2026) showing 14% drop in job-finding for 22-25 year olds in AI-exposed occupations.
		</p>
		<p class="mt-2 text-sm text-muted-foreground">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			...
			company-size modifiers (startup vs enterprise context), and job postings pipeline for real-time
			demand signals.
		</p>
	</section>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			We backtest structural risk scores against actual labour market outcomes at the cluster level
			(Q3 2025 data). This tests whether higher-risk clusters show worse outcomes than lower-risk
			ones.
		</p>
		<div class={cn(card({ padding: 'sm' }), 'mt-3')}>
			<h3 class="text-sm font-semibold text-foreground mb-2">Cluster-Level Directional Checks</h3>
			<div class="space-y-2 text-sm text-muted-foreground">
				<div class="flex items-center gap-2">
					<span class="text-risk-very-low font-bold shrink-0">&#10003;</span>
					<span
						><strong>Risk vs vacancy trend:</strong> Higher-risk clusters have lower vacancy growth (Spearman
						&rho; = -1.0, n=3)</span
					>
				</div>
				<div class="flex items-center gap-2">
					<span class="text-risk-very-low font-bold shrink-0">&#10003;</span>
					<span
						><strong>Risk vs retrenchment:</strong> Higher-risk clusters have higher retrenchment incidence
						(&rho; = 0.5)</span
					>
				</div>
				<div class="flex items-center gap-2">
					...
					<span
						><strong>Risk vs hiring pressure:</strong> Hiring net pressure does not inversely track risk
						(&rho; = 0.5)</span
					>
				</div>
				<div class="flex items-center gap-2">
			...
			<p class="mt-3 text-xs text-muted-foreground italic">
				...
				results in <code class="rounded bg-muted px-1">data/backtests/q3-2025-validation.json</code
				>.
			</p>
		</div>
		<div
			...
			<table class="w-full text-left text-sm">
				...
				<tbody class="text-muted-foreground">
					<tr class="border-b border-border/50">
						...
						<td class="py-2 pr-3"
							>Academic index (Felten et al., 2021) — task-ability overlap with 10 AI applications</td
						>
						<td class="py-2"
					...
					<tr class="border-b border-border/50">
						...
						>
						<td class="py-2"
							>{pct(anthropicCoverageCount, occupationCount)}% of occupations calibrated. Used to
							adjust AIOE where theory diverges from practice.</td
						>
					</tr>
					<tr class="border-b border-border/50">
						...
						>
						<td class="py-2"
							>Pending integration. Crosscheck script ready. Shows as "Exposure consensus" indicator
							when available.</td
						>
					</tr>
				</tbody>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			...
			official SSOC occupation. We estimate scores by blending 2-4 official occupations with weights
			reflecting the typical task composition of each modern role.
		</p>
		<div class="mt-3 space-y-2 text-sm text-muted-foreground">
			<div class={card({ padding: 'sm' })}>
				...
				<p class="mt-1">
					...
					weighted averages of component occupation scores. All component SSOC codes are validated
					against the occupations dataset.
				</p>
			</div>
			<div class={card({ padding: 'sm' })}>
				<p class="font-medium text-foreground">Dispersion</p>
				<p class="mt-1">
					When component occupations have very different risk scores, the weighted average may not
					represent any single reality. For roles with high dispersion (stddev > 0.08), we show the
					full risk range and explain: "Your actual risk depends on how much you code vs manage."
					Low-dispersion roles (where the blend barely changes the score) link to their closest
					official occupation.
				</p>
			</div>
			<div class={card({ padding: 'sm' })}>
	...
	<section class="mb-8 mt-8">
			...
			<table class="w-full text-left text-sm">
				...
				<tbody class="text-muted-foreground">
					...
					<tr class="border-b border-border/50">
						...
						<td class="py-2 pr-3 font-mono">+12pp × sensitivity</td>
						<td class="py-2">
							More judgment/strategy work, higher institutional knowledge. Noy & Zhang (2023): AI
							narrows experience gap. Dell'Acqua (2023): seniors better at knowing AI's capability
							boundary.
						</td>
					</tr>
				</tbody>
		...
		<p class="mt-2 text-xs text-muted-foreground italic">
			...
			requirements. High-sensitivity roles (e.g., software engineering: 0.55) vary more by seniority
			than low-sensitivity roles (e.g., truck driver: 0.15).
		</p>
	</section>
		...
		<div class="mt-3 space-y-2 text-sm text-muted-foreground">
			<div class={card({ padding: 'sm' })}>
				...
				<p class="mt-1 text-xs">
					...
					impact" — but an ensemble of multiple measures accounts for 18% more variation in actual
					outcomes.
				</p>
			</div>
			<div class={card({ padding: 'sm' })}>
				...
				<p class="mt-1 text-xs">
					Brookings/PIIE (2026) reports no consensus on how to measure AI's labour market impact.
					BLS employment projections show only weak correlation between AI exposure and projected
					job decline.
				</p>
			</div>
			<div class={card({ padding: 'sm' })}>
				<p class="font-medium text-foreground/80">No clear aggregate displacement through 2025</p>
				<p class="mt-1 text-xs">
			...
			<div class={card({ padding: 'sm' })}>
				...
				<p class="mt-1 text-xs">
					We measure where AI has the most technical overlap with human tasks, adjusted for human
					bottlenecks and market signals. Validated directionally at cluster level (3/4 checks pass
					vs Q3 2025 data), not causally at occupation level.
				</p>
			</div>
		</div>
	</section>
	...
	<section class="mb-8 mt-8">
		...
		<ul class="mt-2 list-inside list-disc space-y-2 text-sm text-muted-foreground">
			<li>
				<strong>Exposure ≠ displacement</strong> — Market translation uses heuristics and lagging indicators.
				Captures displacement but not reinstatement (Acemoglu &amp; Restrepo, 2019).
			</li>
			<li>
				<strong>US-centric ability data</strong> — O*NET surveys US workers; task composition may differ
				in Singapore.
			</li>
			<li>
			...
			<li>
				<strong>Career-stage blind spot</strong> — Scores the occupation as a whole; junior/senior impact
				likely differs (Stanford Canaries, 2025).
			</li>
			<li>
			...
			<li>
				<strong>Cluster-level labour monitor</strong> — Only available for three broad clusters, not all
				562 occupations.
			</li>
		</ul>
	</section>
	...
	<section class="mb-8 mt-8">
		...
		<p class="mt-2 text-sm text-muted-foreground">
			Key constants and thresholds used in the scoring pipeline. All values are deterministic and
			reproducible.
		</p>
		<div class="mt-3 overflow-x-auto">
			<table class="w-full text-left text-sm">
	...
	<section class="mb-8 mt-8">
		...
		<div class="mt-3 space-y-3">
			...
			<div class={cn(card({ variant: 'inset', padding: 'sm' }))}>
				...
				<p class="mt-1 text-sm text-muted-foreground">
					Added market resilience layer (employment trends + wage scarcity). Introduced risk bands
					and confidence scoring. Replaced single-score output with multi-layer evidence display.
				</p>
			</div>
			<div class={cn(card({ variant: 'inset', padding: 'sm' }))}>
				<div class="flex items-center justify-between">
				...
				<p class="mt-1 text-sm text-muted-foreground">
					Initial release. Two-layer model: AIOE exposure + Pizzinelli theta complementarity. 562
					SSOC occupations via ISCO crosswalk.
				</p>
			</div>
		</div>
	</section>
