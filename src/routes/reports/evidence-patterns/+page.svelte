<script lang="ts">
	...
	const patternMetrics = $derived<Array<[string, number]>>([
		['Pressure above capability', data.coverage.pattern_counts.task_pressure_above_capability_proximity],
		['Capability above pressure', data.coverage.pattern_counts.capability_proximity_above_task_pressure],
...
<main class={pageLayout({ width: 'feature' })}>
	...
	<header class="max-w-4xl">
		...
		<h1 class={cn(title({ size: 'page' }), 'mt-2')}>Where the job signals agree—and where they do not</h1>
	...
	<section class="mt-8 grid gap-px border border-border bg-border sm:grid-cols-4" aria-label="Evidence coverage">
	...
	<section class="mt-12" aria-labelledby="divergence-heading">
		...
		<div class="mt-5 hidden overflow-hidden border border-border bg-card p-4 md:block">
			<svg viewBox="0 0 750 480" class="block h-auto w-full" role="img" aria-labelledby="pattern-chart-title pattern-chart-desc">
				<title id="pattern-chart-title">AI task pressure versus OECD capability proximity</title>
				<desc id="pattern-chart-desc">Seventy-five reviewed Singapore occupation mappings. Each axis is a midrank percentile within the same subset.</desc>
				{#each ticks as tick}
					<line x1={x(tick)} x2={x(tick)} y1={chart.top} y2={chart.top + chart.height} stroke="var(--color-border)" stroke-width="1" />
					<line x1={chart.left} x2={chart.left + chart.width} y1={y(tick)} y2={y(tick)} stroke="var(--color-border)" stroke-width="1" />
					<text x={x(tick)} y={chart.top + chart.height + 22} text-anchor="middle" class="fill-muted-foreground text-[11px]">{tick}</text>
					<text x={chart.left - 12} y={y(tick) + 4} text-anchor="end" class="fill-muted-foreground text-[11px]">{tick}</text>
				{/each}
				<line x1={x(0)} y1={y(0)} x2={x(100)} y2={y(100)} stroke="var(--color-foreground)" stroke-dasharray="5 5" stroke-width="1.5" />
				{#each data.shared as point (point.code)}
					<a href="/occupation/{point.code}">
						<circle cx={x(point.pressure)} cy={y(point.capability)} r={point.demandSources > 0 ? 5.5 : 4} fill={pointColour(point.difference)} opacity="0.84">
							<title>{point.title}: pressure {point.pressure.toFixed(1)}, capability {point.capability.toFixed(1)}, difference {point.difference.toFixed(1)}</title>
						</circle>
					</a>
				{/each}
				<text x={chart.left + chart.width / 2} y="478" text-anchor="middle" class="fill-foreground text-[12px] font-semibold">Task pressure percentile within shared subset</text>
				<text transform="translate(14 234) rotate(-90)" text-anchor="middle" class="fill-foreground text-[12px] font-semibold">Capability proximity percentile within shared subset</text>
			</svg>
			<div class="mt-2 flex flex-wrap gap-4 border-t border-border pt-3 text-xs text-muted-foreground">
			...
			{#each data.mostDivergent.slice(0, 8) as point (point.code)}
				<a href="/occupation/{point.code}" class={card({ padding: 'md', hover: true })}>
					...
					<p class="mt-1 font-mono text-xs text-muted-foreground">
						Pressure {point.pressure.toFixed(1)} · capability {point.capability.toFixed(1)} · gap {point.difference > 0 ? '+' : ''}{point.difference.toFixed(1)}
					</p>
				</a>
			{/each}
		</div>
	...
	<section class="mt-12" aria-labelledby="patterns-heading">
		...
		<h2 id="patterns-heading" class="mt-1 text-2xl font-bold text-foreground">Signals worth investigating</h2>
	...
	<section class="mt-12" aria-labelledby="change-heading">
		...
		<h2 id="change-heading" class="mt-1 text-2xl font-bold text-foreground">Compare each signal at its own cadence</h2>
		<p class="mt-3 max-w-3xl text-sm leading-relaxed text-muted-foreground">{data.changeClaimBoundary}</p>
		<div class="mt-5 grid gap-3 sm:grid-cols-2">
			{#each data.nationalChanges as change (change.key)}
				<div class={card({ padding: 'lg' })}>
					...
					<p class="mt-2 font-mono text-3xl font-bold tabular-nums">{changeText(change.change_pct)}</p>
					<p class="mt-1 text-xs text-muted-foreground">
						{change.from?.value} to {change.to?.value} {change.unit} · {change.from?.period} to {change.to?.period}
					</p>
				</div>
			{/each}
		</div>
		<div class="mt-4 overflow-x-auto border border-border">
			<table class="w-full min-w-[44rem] text-left text-sm">
				<thead class="border-b border-border bg-surface-subtle text-xs text-muted-foreground">
					<tr><th class="px-4 py-3">Broad occupation group</th><th class="px-4 py-3">2025 employment</th><th class="px-4 py-3">Year over year</th><th class="px-4 py-3">Since 2019</th></tr>
				</thead>
				<tbody class="divide-y divide-border">
					{#each data.broadGroupChanges as change (change.key)}
						<tr><td class="px-4 py-3 font-medium text-foreground">{change.label.replace(' resident employment', '')}</td><td class="px-4 py-3 font-mono tabular-nums">{change.latest_value}k</td><td class="px-4 py-3 font-mono tabular-nums">{changeText(change.year_over_year_pct)}</td><td class="px-4 py-3 font-mono tabular-nums">{changeText(change.since_2019_pct)}</td></tr>
					{/each}
				</tbody>
			</table>
		</div>
	...
	<section class="mt-12 grid gap-4 lg:grid-cols-2">
		...
		<div class={card({ padding: 'lg' })}>
			...
			<div class="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm">
				<a href="/data/v9-evidence-vector.json" class="font-medium text-primary underline">Evidence vector JSON</a>
				<a href="/data/v9-signal-change.json" class="font-medium text-primary underline">Change ledger JSON</a>
