...
export const load: PageServerLoad = () => {
	const shared = v9EvidenceVector.records
		...
		.map(record => ({
			...
			difference: record.shared_subset_comparison!.pressure_minus_capability_percentile_points,
	...
	return {
		...
		broadGroupChanges: v9SignalChange.observed_changes.filter(
			change => change.year_over_year_pct != null
		),
