<script lang="ts">
	import { pageLayout, card, sectionLabel, title as titleStyle, caption, mono } from '$lib/design-system';
...
<main class={pageLayout({ width: 'content' })}>
	...
	<div class={cn(card({ padding: 'md', variant: 'notice', accent: 'primary' }), 'mt-4')}>
		...
		<p class="mt-1 text-sm text-muted-foreground">
			...
			{DATA_VINTAGE.occupation_count} occupations and {DATA_VINTAGE.role_count} modern roles.
			V6 baseline scores are preserved in every occupation record for comparison.
		</p>
	</div>
	...
	<section class="mt-8 space-y-6">
			...
			<div class="mt-3 space-y-3 text-sm text-muted-foreground">
				...
				<div class={cn(card({ padding: 'md' }), 'space-y-3')}>
					<div>
						...
						<p class="mt-1">
							Occupations with concentrated AI task exposure face higher displacement risk than those
							with distributed exposure (Hampole et al. 2025). V7 amplifies the base exposure score
							using a task-concentration signal derived from Anthropic task penetration data matched to
					...
					<div class="border-t border-border pt-3">
						...
						<p class="mt-1">
							Addresses the Imas price-elasticity critique: when AI reduces service costs, demand may
							increase enough to offset displacement. Since occupation-level elasticity data does not
							exist, V7 builds a ranked composite proxy from four existing signals.
						</p>
						<div class={cn(card({ padding: 'sm', variant: 'inset' }), 'mt-2 font-mono text-xs')}>
							<p>demand_persistence = 0.4 x momentum_rank + 0.3 x vacancy_rank + 0.2 x scarcity_rank + 0.1 x demand_bonus_rank</p>
							<p>demand_resilience_v7 = clamp01(base_resilience x 0.45 + demand_signal_bonus + 0.10 x demand_persistence)</p>
						</div>
						<p class="mt-2">
							This is not a measure of price elasticity. It is a demand-side counterforce proxy using
							market momentum, vacancy trends, wage scarcity, and official demand signals.
						</p>
					</div>
				</div>
			</div>
		...
		<div>
			...
			<div class={cn(card({ padding: 'md' }), 'mt-3 font-mono text-xs space-y-1')}>
				<p>exposure = reliability-weighted 4-source ensemble (Felten AIOE, Anthropic, Eloundou, ILO)</p>
				<p>task_signal = task_exposure_concentration x task_effective_coverage</p>
				<p>exposure_v7 = clamp01(exposure x (1 + 0.20 x task_signal))</p>
				<p>bottleneck = pctile_rank(Pizzinelli theta)</p>
				<p>displacement_pressure = exposure_v7 x (1 - bottleneck)</p>
				<p>demand_persistence = 0.4 x mm_rank + 0.3 x vacancy_rank + 0.2 x scarcity_rank + 0.1 x bonus_rank</p>
				<p>demand_resilience = clamp01(base_resilience x 0.45 + demand_signal_bonus + 0.10 x demand_persistence)</p>
		...
		<div>
			...
			<div class="mt-3 grid gap-3 sm:grid-cols-2">
				...
				<div class={card({ padding: 'sm' })}>
					...
					<p class={cn(mono({ size: 'sm' }), 'mt-1')}>39 / {DATA_VINTAGE.occupation_count} (6.9%)</p>
		...
		<div>
			...
			<p class="mt-3 text-sm text-muted-foreground">
				V7 passes 166 of 169 structural validation checks. All 6 anchor occupations pass
				(Software Developer, Data Entry Clerk, Surgeon, Telemarketer, Registered Nurse, Data
				Scientist). The 3 pre-existing failures (crosswalk coverage threshold, software developer
				crosswalk type, experimental release status) are unchanged from V6.
			</p>
		</div>
		...
		<div>
			...
			<ul class="mt-3 list-disc pl-5 space-y-2 text-sm text-muted-foreground">
				<li>
					...
					predicts displacement vulnerability. Occupations where AI penetration is concentrated in a
					few high-weight tasks face greater structural pressure than those with distributed exposure.
				</li>
				<li>
					<span class="font-medium text-foreground">Imas / Silicon Canals (2026)</span> — Price
					elasticity of demand is the critical missing variable for predicting actual displacement.
					V7 addresses this with a demand-persistence proxy, not a direct elasticity measure.
				</li>
				<li>
					<span class="font-medium text-foreground">Brookings (2026)</span> — Career pathway erosion
					when gateway occupations are automated. Informs the transition-capacity layer (separate from
					headline score).
				</li>
			</ul>
		</div>
		...
		<div>
			...
			<p class="mt-3 text-sm text-muted-foreground">
				...
				in JSON and CSV formats. V6 baseline scores are preserved in the <code class="text-xs">baseline_v6</code>
