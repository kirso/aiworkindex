		...
		<div class="mt-2 space-y-1">
			{#each bandOrder as band (band)}
				...
				<div class="flex items-center gap-1.5">
					<span class="w-10 shrink-0 text-right text-xs text-muted-foreground">{bandShortLabels[band]}</span>
					...
					<span class="w-6 shrink-0 text-right text-xs tabular-nums text-muted-foreground">{count}</span>
				</div>
			{/each}
		</div>
	</div>
	...
	<div>
		...
		<div class="mt-2 space-y-1">
			{#each impactOrder as type (type)}
				...
				<div class="flex items-center gap-1.5">
					<span class="w-14 shrink-0 text-right text-xs text-muted-foreground">{impactShortLabels[type]}</span>
					<div class="h-3 min-w-0 flex-1 overflow-hidden rounded-full bg-muted">
						<div
							class="h-full rounded-full"
		...
		<ol class="mt-2 space-y-1">
			{#each topHighRisk as occ, i (occ.ssoc)}
				<li class="flex items-start gap-1.5">
					<span class="mt-px flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-rose-100 text-xs font-bold text-rose-600">{i + 1}</span>
	...
	<div>
		...
		<ol class="mt-2 space-y-1">
			{#each topLowRisk as occ, i (occ.ssoc)}
				<li class="flex items-start gap-1.5">
					<span class="mt-px flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-emerald-100 text-xs font-bold text-emerald-600">{i + 1}</span>
					<a href="/occupation/{occ.ssoc}" class="text-xs leading-tight text-foreground hover:text-ring hover:underline">
						{occ.title}
						<span class="ml-1 tabular-nums text-muted-foreground">{(occ.net_risk * 100).toFixed(0)}%</span>
					</a>
	...
	<div>
		...
		<ol class="mt-2 space-y-1">
			{#each theoryVsPractice as occ, i (occ.ssoc)}
				<li class="flex items-start gap-1.5">
					<span class="mt-px flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-secondary text-xs font-bold text-primary">{i + 1}</span>
					<a href="/occupation/{occ.ssoc}" class="text-xs leading-tight text-foreground hover:text-ring hover:underline">
						{occ.title}
