...
const versionTag = DATA_VINTAGE.public_version.toLowerCase().replaceAll('.', '');

const CANONICAL_DATA_FILE = path.join(ROOT_DIR, 'data', 'occupations.json');
const PUBLIC_DATA_FILE = path.join(ROOT_DIR, 'data', 'occupations-v8.json');
...
function runStaticBuildSeoAudit(sitemapLocs: string[]): void {
	...
	for (const page of pages) {
		// SvelteKit emits minimal HTML redirect shims for prerendered redirects.
		// Audit the destination page, not the intentionally metadata-free shim.
		if (/<meta\s+http-equiv=["']refresh["']/i.test(page.html)) continue;
...
const publicData = readJson<Array<Record<string, unknown>>>(PUBLIC_DATA_FILE);
...
assert(
	JSON.stringify(publicData) === JSON.stringify(staticData),
	`static/data/sg-ai-occupations-${versionTag}.json must match the clean public dataset`
);
const appData = readJson<Array<{ v8?: { schema_version?: string } }>>(APP_DATA_FILE);
assert(appData.length === canonicalData.length, 'application dataset must cover every occupation');
assert(
	appData.every(row => row.v8?.schema_version === '8.0'),
	'application dataset missing V8 projections'
);
assert(fs.existsSync(STATIC_CSV_FILE), `missing static/data/sg-ai-occupations-${versionTag}.csv`);
assert(currentCsvHeader.includes('ai_exposure_rank'), 'current CSV missing V8 AI exposure rank');
assert(currentCsvHeader.includes('likely_pathway'), 'current CSV missing V8 likely pathway');
assert(
	currentCsvHeader.includes('evidence_confidence'),
	'current CSV missing V8 evidence confidence'
);
assert(!currentCsvHeader.includes('net_risk'), 'current CSV still exposes legacy net_risk');
assert(
...
assert(claimsMatrix.version === DATA_VINTAGE.public_version, 'claims matrix version drift');
assert(sourceMap.version === DATA_VINTAGE.public_version, 'public field source map version drift');
assert(
...
assert(manifest.version === DATA_VINTAGE.public_version, 'release manifest version drift');
assert(
...
for (const [name, contents] of [
	...
	assert(
		contents.includes(DATA_VINTAGE.public_version),
		`${name} missing current ${DATA_VINTAGE.public_version} vintage`
	);
	...
	assert(contents.includes(DATA_VINTAGE.public_version), `${name} missing current release tag`);
}
...
assert(sitemapLocs.length > 600, 'sitemap is unexpectedly small');
...
assert(!sitemap.includes('/us/occupation/'), 'sitemap includes withdrawn US occupation scores');
assert(
	!sitemap.includes('/global/occupation/'),
	'sitemap includes withdrawn global occupation scores'
);
assert(!sitemap.includes('/us/role/'), 'sitemap includes withdrawn US role scores');
...
const ogOccupations = JSON.parse(fs.readFileSync(APP_DATA_FILE, 'utf-8')) as Array<{
	ssoc: string;
	net_risk: number;
	risk_band: string;
}>;
