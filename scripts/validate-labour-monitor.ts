...
const EXPECTED_CLUSTERS: ClusterKey[] = [
  "pmet",
...
function main() {
  ...
  for (const m of monitors) {
    ...
    if (!Array.isArray(quarters) || quarters.length === 0) {
      ...
      for (let i = 1; i < quarters.length; i++) {
        if (
          quarterSortKey(quarters[i].quarter) <=
          quarterSortKey(quarters[i - 1].quarter)
        ) {
          ordered = false;
          break;
        }
      ...
      const allZero = quarters.every(
        (q: any) => q.rate === 0 || q.rate === null
      );
      if (allZero) {
    ...
    // Overall signal
    if (
      !["strong", "moderate", "weak", "deteriorating"].includes(m.overall)
    ) {
  ...
  if (uniqueOveralls.size === 1) {
    warn(
      `All clusters have same overall signal: ${overalls[0]} — may indicate data issue`
    );
  }
