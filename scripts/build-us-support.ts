...
function parseSkillsProfiles(): Map<string, SupportDemandProfile & SupportSkillsProfile> {
	...
	for (const row of rows.slice(2)) {
		...
		const skills = [
			cleanText(String(row[10] ?? '')),
			cleanText(String(row[11] ?? '')),
			cleanText(String(row[12] ?? ''))
		...
		profiles.set(code, {
			...
			outlook: null,
			relatedOOHContent:
				relatedOOHContent && relatedOOHContent !== 'OOH Content' ? relatedOOHContent : null,
			topSkills: skills
		});
	}
...
function parseOohProfiles(): Map<string, SupportNarrativeProfile> {
	...
	for (const match of xml.matchAll(/<soc_code type="text">([\s\S]*?)<\/soc_code>/g)) {
		...
		const similarSection =
			block.match(/<similar_occupations>([\s\S]*?)<\/similar_occupations>/i)?.[1] ?? '';
...
> {
	...
	const sheet =
		workbook.Sheets[
			workbook.SheetNames.find(name => name.includes('dataset')) ?? workbook.SheetNames[0] ?? ''
		];
	const rows = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '' });
	...
	for (const row of rows) {
		...
		bucket.items.push({
			...
			detail: occupation
				? `${occupation}${requirement ? ` · ${requirement}` : ''} · ${estimateText}`
				: `${requirement || estimateText}`,
	...
	for (const [code, buckets] of profiles) {
		for (const bucket of buckets) {
			bucket.items.sort((left, right) => {
				const leftIndex = priority.findIndex(token =>
					`${left.label} ${left.detail ?? ''}`.toLowerCase().includes(token)
				);
				const rightIndex = priority.findIndex(token =>
					`${right.label} ${right.detail ?? ''}`.toLowerCase().includes(token)
				);
				return (
...
function jobZoneSummary(zone: number | null): string | null {
	...
	if (zone === 5)
		return 'The occupation usually needs extensive preparation, training, and experience.';
	return null;
}
...
function buildSupportEntries(): UnitedStatesSupportEntry[] {
	...
	return occupations.map(occupation => {
		...
		const topTechnologies = [...(technologiesBySoc.get(soc) ?? [])]
			...
			.sort(
				(a, b) =>
					...
					Number(b.inDemand) - Number(a.inDemand) ||
		...
		const narrativeProfile = oohProfiles.get(soc) ??
			oohProfiles.get(socKey) ??
			oohProfiles.get(canonicalSoc) ?? {
		...
		const orsBuckets =
			orsProfiles.get(canonicalSoc) ?? orsProfiles.get(socKey) ?? orsProfiles.get(soc) ?? [];
		const requirementProfile =
			orsBuckets
				.slice()
				.sort(
					(left, right) =>
						titleSimilarity(occupation.localTitle, right.occupation) -
						titleSimilarity(occupation.localTitle, left.occupation)
				)[0]?.items ?? [];
		...
		return {
			...
			topWorkContext,
			ageProfile,
			note: 'Built from O*NET occupation descriptions, task statements, technology skills, work context, Job Zones, Anthropic task penetration, BLS OEWS wages, BLS projection tables, BLS ORS requirements, BLS OOH narrative content, BLS skills data, and BLS CPS occupation age tables.',
			sourceVintage:
...
function main() {
	...
	const payload = {
		...
		source_vintage:
			'O*NET 30.2 / OEWS 2024 / ORS 2025 / OOH 2025-08-28 / Projections 2024-34 / CPS 2025 / Anthropic task penetration',
		note: 'US evidence-support bundle built from the checked-in public raw corpus. This is context, not a direct labor-market outcome.',
		entries
	};
