...
function parseVacancyRates(): Map<
  ClusterKey,
  Array<{ quarter: string; rate: number }>
> {
  ...
  // Find column indices
  const quarterIdx = header.findIndex((h) =>
    h.toLowerCase().includes("quarter")
  );
  const industry1Idx = header.findIndex((h) =>
    h.toLowerCase().includes("industry1")
  );
  const industry2Idx = header.findIndex((h) =>
    h.toLowerCase().includes("industry2")
  );
  const occupationIdx = header.findIndex((h) =>
    h.toLowerCase().includes("occupation")
  );
  const rateIdx = header.findIndex((h) =>
    h.toLowerCase().includes("vacancy_rate") || h.toLowerCase().includes("job_vacancy_rate")
  );
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (fields.length < Math.max(quarterIdx, rateIdx, occupationIdx) + 1)
      continue;
    ...
    const occLabel = fields[occupationIdx]
      ?.toLowerCase()
      .replace(/"/g, "")
      .trim();
    ...
    for (const [key, csvName] of Object.entries(CLUSTER_CSV_NAMES) as Array<
      [ClusterKey, string]
    >) {
      if (occLabel === csvName) {
...
> | null {
  ...
  if (!fs.existsSync(filePath)) {
    console.log(
      "  INFO: recruitment_resignation_rates.csv not found, skipping hiring signal"
    );
    return null;
  }
  ...
  try {
    ...
    console.log(
      "  Found recruitment/resignation CSV, parsing..."
    );
...
function parseRetrenchment(): Map<
  ClusterKey,
  Array<{ quarter: string; count: number }>
> | null {
...
function computeVacancySignal(
  ...
  // 4Q-over-4Q average trend
  const latest4 = series.slice(0, 4);
  const prev4 = series.slice(4, 8);

  let trend4qPct = 0;
  if (prev4.length === 4 && latest4.length === 4) {
    const avgLatest =
      latest4.reduce((s, p) => s + p.rate, 0) / latest4.length;
    const avgPrev = prev4.reduce((s, p) => s + p.rate, 0) / prev4.length;
    if (avgPrev > 0) {
      trend4qPct = (avgLatest / avgPrev - 1) * 100;
    }
  }

  // Signal: trend > +5% -> +1, < -5% -> -1, else 0
  let signal: 1 | 0 | -1 = 0;
...
function computeOverallSignal(
  ...
  if (hiringSignal !== null) total += hiringSignal;
  if (retrenchmentSignal !== null) total += retrenchmentSignal;
...
function main() {
  ...
  for (const clusterKey of Object.keys(CLUSTER_CSV_NAMES) as ClusterKey[]) {
    ...
    if (!vacancy) {
      console.error(
        `ERROR: Could not compute vacancy signal for ${clusterKey}`
      );
      process.exit(1);
    }
